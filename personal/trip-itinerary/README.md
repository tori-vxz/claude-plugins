...
Give it an ordered list of cities and it produces one itinerary covering all three activity levels (low, medium, high), each showing all three travel budget tiers (cheapest, mid-range, luxury) for every leg, with a link on every activity and every travel option. The deliverable is always a spreadsheet, one tab per activity level — either an `.xlsx` workbook or a self-contained web page for a Claude artifact, whichever is asked for.
